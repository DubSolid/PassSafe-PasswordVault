...
def item_listing(db: Database, user_id):
    ...
    if not items:
        ...
        listing_message = '(+) Press Enter to continue...'     
        seperator = '-' * len(listing_message)
        input(f'\n{seperator}\n{tcolors.GREEN(listing_message)}\n{seperator}')
...
def item_retrieval(db: Database, user_id):
    ...
    while True:
            ...
            if item_number < 1 or item_number > len(items):
                ...
                print(f'\n'
                      f'Item Name: {tcolors.GREEN(str(selected_item[1]))}\n'
                      f'Username/e-mail address: {tcolors.GREEN(str(selected_item[2]))}\n'
                      f'Password: {tcolors.GREEN(str(selected_item[3]))}\n'
                      f'URL: {tcolors.GREEN(str(selected_item[4]))}\n'
                      f'Notes: {tcolors.GREEN(str(selected_item[5]))}')  
                copy_message = '(+) Please select item information you need and copy it to your clipboard.'     
                seperator = '-' * len(copy_message)
                print(f'\n{seperator}\n{tcolors.GREEN(copy_message)}\n{seperator}')
